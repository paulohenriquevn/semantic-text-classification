...
### Added
- Dissertation Chapter 3 (Related Work) written to `mit/chapter3_related_work.md`: covers hybrid retrieval systems (Rayo et al. 2025), lexical vs semantic search (Harris 2025), LLM-based clustering (Huang & He 2025), attention mechanisms for classification (Lyu et al. 2025), embeddings-plus-classifier paradigm (AnthusAI 2024), contact-center NLP (BERTaú 2021), rule-based systems (SystemT, Rasa), and a systematic 8-work × 6-dimension gap analysis arguing why no prior work combines hybrid retrieval, multi-level classification, and auditable rules on conversational PT-BR data (no-ticket)
- Leave-One-Domain-Out (LODO) cross-domain evaluation script (`experiments/scripts/run_lodo_experiment.py`): runs 8 LODO folds over the dataset domains (financeiro, restaurante, saude, imobiliario, telecom, ecommerce, tecnologia, educacao), training LightGBM with lexical+structural+embedding features and context windows (5t/2s) on 7 domains and evaluating on the held-out domain; reports Macro-F1, accuracy, per-class F1 per fold and mean±std summary with generalization gap; results saved to `experiments/results/LODO/results.json` (no-ticket)
- Fine-tune vs frozen comparison script (`experiments/scripts/finetune_baseline.py`): fine-tunes paraphrase-multilingual-MiniLM-L12-v2 via MultipleNegativesRankingLoss on same-intent pairs, evaluates three conditions (frozen + LightGBM, fine-tuned + LightGBM, fine-tuned + linear head) with context windows 5t/2s and conversation-level aggregation, reports Macro-F1 and per-class F1 for each, saves structured results to `experiments/results/finetune_comparison/results.json`; CLI supports `--splits-dir`, `--epochs`, `--batch-size`, `--lr`, `--seed`, `--output-dir`, `--model-dir`, `--skip-finetune` (no-ticket)
- Per-class error analysis script (`experiments/scripts/error_analysis.py`) reproducing the best H2 model and producing: confusion matrices (raw + normalized), top-3 confusion targets per class, misclassified conversation samples for weak classes, LightGBM gain-based feature importances, class separability via embedding centroids, origin breakdown per class (synthetic vs original), confidence analysis (correct vs incorrect predictions), and symmetric most-confused-pair ranking; unified `results.json` saved to `experiments/results/error_analysis/`; CLI supports both `--splits-dir` and `--data-dir` options (no-ticket)
- Calibration metrics (Brier score + ECE) added to H2, H3, and ablation experiments via `_compute_calibration_metrics` and `_aggregate_windows_to_conversations_with_probs` (no-ticket)
- Stratified k-fold cross-validation script (`experiments/scripts/run_kfold_experiment.py`): pools all splits, runs stratified 5-fold CV with full pipeline (lexical+structural+embedding+rules LightGBM), reports mean±std Macro-F1 with real confidence intervals (no-ticket)
- Expanded experiment ruleset from 2 to 10 rules via shared `_build_experiment_rules()`: covers all 8 intent classes, uses 3 predicate families (LEXICAL, STRUCTURAL, CONTEXTUAL) including regex, near proximity, repeated_in_window, and occurs_after patterns (no-ticket)

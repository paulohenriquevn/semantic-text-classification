...
Esta hipótese estende o trabalho de Lyu et al. (2025) sobre mecanismos de atenção para classificação, aplicando a intuição de que conversas multi-turn possuem informação distribuída em diferentes granularidades. O turno captura a intenção local, a janela de contexto captura dependências temporais, e a conversa completa captura o desfecho global. Nenhum nível isolado contém toda a informação necessária. O princípio "embeddings representam, classificadores decidem" [AnthusAI] é preservado: os embeddings multi-nível alimentam classificadores supervisionados (regressão logística, gradient boosting, MLP), não são usados diretamente como classificadores via similaridade. O critério de confirmação exige que a melhor configuração multi-nível supere todas as configurações de nível único em Macro-F1 com significância estatística (p < 0,05) em pelo menos 60% das classes.

...
def _make_training_data() -> tuple[list[ClassificationInput], list[str]]:
    """Create training data with clear separation for neural models.

    Uses more samples than the logistic test to ensure the MLP converges
    reliably on small hidden layers.
    """
...
MLP_KWARGS = {"hidden_layer_sizes": (16, 8), "max_iter": 2000, "random_state": 42, "learning_rate_init": 0.01}
...
class TestMLPClassification:
    ...
    def test_model_name_and_version_in_result(self) -> None:
        ...
        results = cls.classify([test_input])
        assert results[0].model_name == "my-mlp"
